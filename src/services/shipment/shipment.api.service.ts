...
export type SEGMENT_STATUS = typeof SEGMENT_STATUS[keyof typeof SEGMENT_STATUS];
...
const segmentReadDtoSchema = z.object({
  id: z.string().uuid(),
  ...
  currentLatitude: z.union([z.number(), z.string()]).optional().nullable().transform((val) => val === null || val === undefined ? null : (typeof val === 'string' ? parseFloat(val) : val)),
  currentLongitude: z.union([z.number(), z.string()]).optional().nullable().transform((val) => val === null || val === undefined ? null : (typeof val === 'string' ? parseFloat(val) : val)),
...
const shipmentReadDtoSchema = z.object({
  id: z.string().uuid(),
  title: z.string(),
  ...
  cargoType: z.string(),
  cargoWeight: z.union([z.number(), z.string()]).transform((val) => typeof val === 'string' ? parseFloat(val) : val),
...
const activityLogReadDtoSchema = z.object({
  id: z.string().uuid(),
...
const updateShipmentDtoSchema = z.object({
 ...
 */
export async function createShipment(data: CreateShipmentDto): Promise<ShipmentReadDto> {
  try {
    // Validate input
    const validatedData = createShipmentDtoSchema.parse(data);
    
    const response = await http.post<ShipmentReadDto>("/shipments", validatedData);
 ...
 */
export async function listShipments(filters: ShipmentFilters = {}): Promise<ShipmentReadDto[]> {
  try {
    ...
    try {
      ...
      if (validationError instanceof z.ZodError) {
        ...
        throw new Error(`Invalid response format: ${firstIssue?.path.join('.')} - ${firstIssue?.message}`);
      }
 ...
 * Update a shipment
 */
export async function updateShipment(id: string, data: UpdateShipmentDto): Promise<ShipmentReadDto> {
  try {
    // Validate input
    const validatedData = updateShipmentDtoSchema.parse(data);
    
    const response = await http.put<ShipmentReadDto>(`/shipments/${id}`, validatedData);
    return shipmentReadDtoSchema.parse(response.data);
  } catch (error: unknown) {
    if (error instanceof z.ZodError) {
      const firstError = error.issues[0];
 ...
 */
export async function getShipmentSegments(id: string): Promise<SegmentReadDto[]> {
  try {
    const response = await http.get<SegmentReadDto[]>(`/shipments/${id}/segments`);
 ...
 */
export async function getShipmentActivityLog(id: string): Promise<ActivityLogReadDto[]> {
  try {
    const response = await http.get<ActivityLogReadDto[]>(`/shipments/${id}/activity-log`);
